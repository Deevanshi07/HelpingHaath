...
app.post('/api/campaigns', async (req, res) => {
  try {
    const { title, description, goal, coverUrl, createdBy } = req.body || {};
    const rupees = Number(goal || 0);
    if (!title || !rupees || rupees <= 0) {
      return res.status(400).json({ error: 'title and positive goal required' });
    }
    ...
    res.status(201).json({ ok: true, campaign: doc });
  } catch (e) {
    res.status(400).json({ error: e.message });
  }
});
